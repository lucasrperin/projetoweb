...
$json = file_get_contents("http://192.168.0.105/web/Sistemas/json_cliente.php");
...
if (count($obj) > 0) {
    ...
    while ($i < count($obj)) {
        $username = mysqli_real_escape_string($cone, $obj[$i]->nome); // Escapar valor

        // Montar a query corretamente
        $SQL  = "INSERT INTO clientes (ds_cliente) VALUES ('$nome')";

        if ($cone->query($SQL) === TRUE) {
            echo "<tr><td>Inserido: $nome</td></tr>";
        } else {
